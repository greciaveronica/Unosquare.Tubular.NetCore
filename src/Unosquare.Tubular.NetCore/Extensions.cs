                ...
                switch (column.Aggregate)
                {
                    case AggregationFunction.Sum:
                        ...
                        else
                        {
                            ...
                            {
                                if (subset.ElementType.GetProperty(column.Name).PropertyType == typeof(decimal))
                                {
                                    response.AggregationPayload.Add(column.Name,
                    ...
                    case AggregationFunction.Count:
                        response.AggregationPayload.Add(column.Name, subset.Select(column.Name).Cast<object>().Count());
                        break;
                    case AggregationFunction.DistinctCount:
                        response.AggregationPayload.Add(column.Name,
                            subset.Select(column.Name).Cast<object>().Distinct().Count());
                        break;
